...
LongNumber LongNumber::operator&(LongNumber& other) const
{
    LongNumber result;
    for (int i = 0; i < NUMBER_LENGTH; i++) {
        result.number_array[i] = this->number_array[i] & other.number_array[i];
    }
    return result;
}
...
LongNumber convertToNumber(std::string hex)
{
    ...
    for (int i = 0; i < hex.length(); i++) {
        uint32_t modulo = i % 8;
        uint32_t digit = std::stoi(hex.substr(hex.length() - i - 1, 1), 0, 16);
        result.number_array[i / 8] += digit * (1 << (4 * modulo));
    }
    return result;
}

std::string LongNumber::toHex() const
{
    const std::string hexs = "0123456789abcdef";

    LongNumber number(*this);
    std::string hex = "";
    for (int i = 0; i < NUMBER_LENGTH * 8; i++) {
        uint32_t modulo = i % 8;
        hex += hexs[(number.number_array[i / 8] >> (4 * modulo) & 15)];
    }
